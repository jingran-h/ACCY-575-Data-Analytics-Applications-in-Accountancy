...
2. **Optional:** Apply for [GitHub Education](https://education.github.com/) — verified students **may qualify** for free GitHub Copilot as an AI coding assistant for the course. (Heads up: as of mid-2026 GitHub has **paused new sign-ups** for the free *GitHub Copilot Student* plan — students already enrolled keep it, but new sign-ups currently land on the free Copilot tier only. Tiers and eligibility change over time, so check the current GitHub Education terms before counting on it; either way it's optional — the course works with any AI assistant, or none.) You'll need an `@illinois.edu` email on your account for student verification — add it under *Settings → Emails* if it's not there yet. Approval can take up to 48 hours, so start early if you want it.
